...
try:
    ...
    # Fallback to simple logic if util missing
    def calculate_wheel_speeds(s, a, w, r=-1): return s, s, s, s
...
def steer(angle=190., speed=0.8):
    # --- BOZOTICS OMNI-ADAPTER IMPLEMENTATION ---
    # Adapts Holonomic Math to Differential Drive for smoother control
    
    if angle == 190: # STOP
        forward_right.off(); backward_right.off()
        forward_left.off(); backward_left.off()
        speed_left.value = 0; speed_right.value = 0
        return
        
    if angle == 200: # BACKWARD (Inverted Logic: Forward pins = Backward)
        forward_right.on(); backward_right.off()
        forward_left.on(); backward_left.off()
        speed_left.value = max(speed * left_correction, 0)
        speed_right.value = max(speed * right_correction, 0)
        return
        
    # DRIVE (Forward biasing with Rotation)
    # Angle input (-180 to 180) is treated as Heading Error (Rotation requirement)
    # Speed input (0-1) is Linear Velocity
    
    s_val = speed * 1000
    r_val = angle * 8.0 # Gain for rotation sensitivity (tuned for Bozotics feel)
    
    # Calculate Omni Vectors (Angle=0 means "Drive Forward" in robot frame)
    fl, fr, bl, br = calculate_wheel_speeds(s_val, 0, r_val)
    fl, fr, bl, br = normalize_speeds(fl, fr, bl, br, 1000)
    
    # Map to Differential (Average Lefts / Average Rights)
    l_out = (fl + bl) / 2.0
    r_out = (fr + br) / 2.0
    
    # Apply to Motors (Inverted Logic: Backward_Pins = Forward Direction)
    
    # LEFT MOTOR
    if l_out >= 0:
        forward_left.off(); backward_left.on() # Drive Fwd
    else:
        forward_left.on(); backward_left.off() # Drive Bkwd (Pivot)
    speed_left.value = min(abs(l_out)/1000.0 * left_correction, 1.0)
    
    # RIGHT MOTOR
    if r_out >= 0:
        forward_right.off(); backward_right.on() # Drive Fwd
    else:
        forward_right.on(); backward_right.off() # Drive Bkwd
    speed_right.value = min(abs(r_out)/1000.0 * right_correction, 1.0)
